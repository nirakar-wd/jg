...
const validatePassword = (req, res, next) => {
  ...
  if (!req.body.newpassword || !validatePasswordReg) {
    helpers.customErrorResponse(
      res,
      400,
      "gunakan alpha numeric, 1 uppercase, 1 lowercase, min 6 character"
    );
  }
...
const validateSignup = (req, res, next) => {
  ...
  if (
    ...
    !regexField.test(req.body.username)
  ) {
    helpers.customErrorResponse(
      res,
      400,
      "Please enter a username with min. 3 chars without single quote"
    );
  } else if (!req.body.useremail || !validateEmailReg) {
    helpers.customErrorResponse(res, 400, "Please enter a valid email");
  } else if (
    (req.body.account_type === "seller" || req.body.account_type === "1") &&
    (!req.body.user_phone ||
      req.body.user_phone.length < 9 ||
      req.body.user_phone.length > 13 ||
      req.body.user_phone.substr(0, 1) !== "8" ||
      isNaN(req.body.user_phone))
  ) {
    helpers.customErrorResponse(
      res,
      400,
      "Please enter valid phone number, numeric, 8xxxx..., min 9, max 13 character"
    );
  } else if (
    (req.body.account_type === "seller" || req.body.account_type === "1") &&
    (!req.body.userstore || req.body.userstore.length < 3 || !validateStringReg)
  ) {
    helpers.customErrorResponse(
      res,
      400,
      "Please enter a store name with min. 3 chars with no singgle quotes"
    );
  } else if (!req.body.password || !validatePasswordReg) {
    helpers.customErrorResponse(
      res,
      400,
      "Use alpha numeric, 1 uppercase, 1 lowercase, min 6 character"
    );
  } else {
    next();
  }
